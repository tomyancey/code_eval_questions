...
file = File.open "compressed_sequence.txt"

def compress_sequence(str)
    arr = []
    current_val = nil
    current_total = 0
    str.split(" ").each do |val|
        if current_val.nil?
            ...
            arr.push(current_total,current_val)
            current_val = val
            current_total = 0
        end
        current_total += 1
...
file.each_line {|line| puts compress_sequence(line.chomp) }
